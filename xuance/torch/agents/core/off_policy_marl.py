    ...
    def train(self, n_steps):
        ...
        for _ in tqdm(range(n_steps)):
            ...
            for i in range(self.n_envs):
                ...
                episode_voltage_violaton = episode_voltage_violaton + info[i]['total_violation']
                episode_sum_reward = episode_sum_reward + info[i]['mean_reward']
                ...
                if all(terminated_dict[i].values()) or truncated[i]:
                    ...
                    if self.use_wandb:
                        ...
                        self.log_voltage_violaton.append([self.current_step, episode_voltage_violaton])
                        self.log_episode_mean_reward.append([self.current_step, episode_sum_reward / 96])
    ...
    def run_episodes(self, env_fn=None, n_episodes: int = 1, test_mode: bool = False):
        ...
        while episode_count < n_episodes:
            ...
            for i in range(num_envs):
                ...
                # 手动记录数据
                episode_power_loss = episode_power_loss + info[i]['power_loss']
                episode_voltage_violation = episode_voltage_violation + info[i]['total_violation']
                episode_sum_reward = episode_sum_reward + info[i]['mean_reward']
                    ...
                    if test_mode:
                        ...
                        if self.use_wandb:
                            ...
                            self.log_reward.append([self.current_step, np.mean(
                                itemgetter(*self.agent_keys)(info[i]["episode_score"]))])
                            self.log_powerloss.append([self.current_step, episode_power_loss])
                            self.log_voltage_violaton.append([self.current_step, episode_voltage_violation])
                            self.log_episode_mean_reward.append([self.current_step, episode_sum_reward / 96])
